...
#include "VolumeAsset/DICOMParser/DICOMCallback.h"
...
{
	bool operator()(const std::pair<int, std::string>& s1, const std::pair<int, std::string>& s2) const
	{
...
{
	bool operator()(const std::pair<float, std::string>& s1, const std::pair<float, std::string>& s2) const
	{
		return s1.first < s2.first;
	}
};
...
void DICOMAppHelper::RegisterCallbacks(DICOMParser* parser)
{
	...
	parser->AddDICOMTagCallback(0x0028, 0x0100, DICOMParser::VR_UnsignedShort, BitsAllocatedCB);
	...
	parser->AddDICOMTagCallback(0x0028, 0x0030, DICOMParser::VR_Float, PixelSpacingCB);
	parser->AddDICOMTagCallback(0x0018, 0x0050, DICOMParser::VR_Float, PixelSpacingCB);

	WidthCB->SetCallbackFunction(this, &DICOMAppHelper::WidthCallback);
	parser->AddDICOMTagCallback(0x0028, 0x0011, DICOMParser::VR_UnsignedShort, WidthCB);

	HeightCB->SetCallbackFunction(this, &DICOMAppHelper::HeightCallback);
	parser->AddDICOMTagCallback(0x0028, 0x0010, DICOMParser::VR_UnsignedShort, HeightCB);

	PixelRepresentationCB->SetCallbackFunction(this, &DICOMAppHelper::PixelRepresentationCallback);
	parser->AddDICOMTagCallback(0x0028, 0x0103, DICOMParser::VR_UnsignedShort, PixelRepresentationCB);
	...
	parser->AddDICOMTagCallback(0x0028, 0x1053, DICOMParser::VR_Float, RescaleSlopeCB);
	...
	parser->AddDICOMTagCallback(0x0018, 0x1120, DICOMParser::VR_Float, GantryAngleCB);

	DICOMTagInfo dicom_tags[] = {{0x0002, 0x0002, DICOMParser::VR_UI, "Media storage SOP class uid"},
		{0x0002, 0x0003, DICOMParser::VR_UI, "Media storage SOP inst uid"},
		{0x0002, 0x0010, DICOMParser::VR_UI, "Transfer syntax uid"},
		{0x0002, 0x0012, DICOMParser::VR_UI, "Implementation class uid"}, {0x0008, 0x0018, DICOMParser::VR_UI, "Image UID"},
		{0x0008, 0x0020, DICOMParser::VR_DA, "Series date"}, {0x0008, 0x0030, DICOMParser::VR_TM, "Series time"},
		{0x0008, 0x0060, DICOMParser::VR_SH, "Modality"}, {0x0008, 0x0070, DICOMParser::VR_SH, "Manufacturer"},
		{0x0008, 0x1060, DICOMParser::VR_SH, "Physician"}, {0x0018, 0x0050, DICOMParser::VR_Float, "slice thickness"},
		{0x0018, 0x0060, DICOMParser::VR_Float, "kV"}, {0x0018, 0x0088, DICOMParser::VR_Float, "slice spacing"},
		{0x0018, 0x1100, DICOMParser::VR_SH, "Recon diameter"}, {0x0018, 0x1151, DICOMParser::VR_Float, "mA"},
		{0x0018, 0x1210, DICOMParser::VR_SH, "Recon kernel"}, {0x0020, 0x000d, DICOMParser::VR_UI, "Study UID"},
		{0x0020, 0x000e, DICOMParser::VR_UI, "Series UID"}, {0x0020, 0x0013, DICOMParser::VR_IS, "Image number"},
		{0x0020, 0x0032, DICOMParser::VR_SH, "Patient position"}, {0x0020, 0x0037, DICOMParser::VR_SH, "Patient position cosines"},
		{0x0020, 0x1041, DICOMParser::VR_CS, "Slice location"}, {0x0028, 0x0010, DICOMParser::VR_Float, "Num rows"},
		{0x0028, 0x0011, DICOMParser::VR_Float, "Num cols"}, {0x0028, 0x0030, DICOMParser::VR_Float, "pixel spacing"},
		{0x0028, 0x0100, DICOMParser::VR_UnsignedShort, "Bits allocated"}, {0x0028, 0x0120, DICOMParser::VR_UnsignedLong, "pixel padding"},
		{0x0028, 0x1052, DICOMParser::VR_Float, "pixel offset"}};
	...
	DICOMParser* parser, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte)
{
	...
	if (iter == this->Implementation->SeriesUIDMap.end())
	{
		...
		this->Implementation->SeriesUIDMap.insert(std::pair<const std::string, std::vector<std::string>>(newStdString, newVector));
	}
	else
	{
...
void DICOMAppHelper::OutputSeries()
{
	...
	for (std::map<std::string, std::vector<std::string>, ltstdstr>::iterator iter = this->Implementation->SeriesUIDMap.begin();
...
void DICOMAppHelper::ArrayCallback(
	DICOMParser* parser, doublebyte group, doublebyte element, DICOMParser::VRTypes datatype, unsigned char* val, quadbyte len)
{
	const char* desc = "No description";

	TagMapType::iterator iter = this->Implementation->TagMap.find(std::pair<doublebyte, doublebyte>(group, element));
	...
	*this->HeaderFile << desc << " : ";
	...
	if (val)
	{
		switch (datatype)
		{
			...
			case DICOMParser::VR_WordString:	// ordered words
			...
			case DICOMParser::VR_IS:
				*this->HeaderFile << val;
				break;
			case DICOMParser::VR_Float:	// float
				fval = static_cast<float>(atof(reinterpret_cast<char*>(val)));
				*this->HeaderFile << fval;
				break;
			case DICOMParser::VR_Double:	// float double
				dval = static_cast<double>(atof(reinterpret_cast<char*>(val)));
				*this->HeaderFile << dval;
				break;
			case DICOMParser::VR_UnsignedLong:	// unsigned long
			case DICOMParser::VR_SignedLong:	// signed long
			...
			case DICOMParser::VR_SignedShort:
				...
				*this->HeaderFile << ival;
				break;
			case DICOMParser::VR_UnsignedShort:	// unsigned short
...
//
void DICOMAppHelper::PixelSpacingCallback(
	DICOMParser* parser, doublebyte group, doublebyte element, DICOMParser::VRTypes, unsigned char* val, quadbyte)
{
	...
	{
		this->PixelSpacing[2] = DICOMFile::ReturnAsFloat(val, parser->GetDICOMFile()->GetPlatformIsBigEndian());
	}
}
	...
	DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* data, quadbyte len)
{
	...
	if (isFloat)
	{
		...
		this->ImageDataType = DICOMParser::VR_Float;
		...
		if (ptrIncr == 1)
		{
			...
			{
				newFloatPixel =
					float(static_cast<double>(this->RescaleSlope) * ucharInputData[i] + static_cast<double>(this->RescaleOffset));
		...
		else if (ptrIncr == 2)
		{
			...
			{
				newFloatPixel =
					float(static_cast<double>(this->RescaleSlope) * ushortInputData[i] + static_cast<double>(this->RescaleOffset));
				floatOutputData[i] = newFloatPixel;
			}
#ifdef DEBUG_DICOM_APP_HELPER
	...
	else
	{
		...
		if (ptrIncr == 1)
		{
			...
			{
				newCharPixel =
					char(static_cast<double>(this->RescaleSlope) * ucharInputData[i] + static_cast<double>(this->RescaleOffset));
		...
		else if (ptrIncr == 2)
		{
			...
			this->ImageDataType = DICOMParser::VR_WordString;
			unsigned long uNumPixels = static_cast<unsigned long>(numPixels);
			...
			{
				newShortPixel =
					short(static_cast<double>(this->RescaleSlope) * shortInputData[i] + static_cast<double>(this->RescaleOffset));
...
void DICOMAppHelper::RegisterPixelDataCallback(DICOMParser* parser)
{
	...
	parser->AddDICOMTagCallback(0x7FE0, 0x0010, DICOMParser::VR_WordString, this->PixelDataCB);
}
	...
	const std::string& seriesUID, std::vector<std::pair<int, std::string>>& v, bool ascending)
{
	...
	std::map<std::string, std::vector<std::string>, ltstdstr>::iterator miter = this->Implementation->SeriesUIDMap.find(seriesUID);
	...
	for (std::vector<std::string>::iterator fileIter = files.begin(); fileIter != files.end(); ++fileIter)
	{
		...
		int slice_number;
		std::map<std::string, DICOMOrderingElements, ltstdstr>::iterator sn_iter = Implementation->SliceOrderingMap.find(*fileIter);
...
void DICOMAppHelper::GetSliceNumberFilenamePairs(std::vector<std::pair<int, std::string>>& v, bool ascending)
{
...
void DICOMAppHelper::GetSliceLocationFilenamePairs(std::vector<std::pair<float, std::string>>& v, bool ascending)
{
	// Default to using the first series
	if (!this->Implementation->SeriesUIDMap.empty())
	{
		this->GetSliceLocationFilenamePairs((*this->Implementation->SeriesUIDMap.begin()).first, v, ascending);
	}
	else
	{
		v.clear();
	...
	const std::string& seriesUID, std::vector<std::pair<float, std::string>>& v, bool ascending)
{
	...
	for (std::vector<std::string>::iterator fileIter = files.begin(); fileIter != files.end(); ++fileIter)
	{
		...
		if (sn_iter != Implementation->SliceOrderingMap.end())
		{
			...
			normal[0] = ((*sn_iter).second.ImageOrientationPatient[1] * (*sn_iter).second.ImageOrientationPatient[5]) -
						((*sn_iter).second.ImageOrientationPatient[2] * (*sn_iter).second.ImageOrientationPatient[4]);
			normal[1] = ((*sn_iter).second.ImageOrientationPatient[2] * (*sn_iter).second.ImageOrientationPatient[3]) -
						((*sn_iter).second.ImageOrientationPatient[0] * (*sn_iter).second.ImageOrientationPatient[5]);
			normal[2] = ((*sn_iter).second.ImageOrientationPatient[0] * (*sn_iter).second.ImageOrientationPatient[4]) -
						((*sn_iter).second.ImageOrientationPatient[1] * (*sn_iter).second.ImageOrientationPatient[3]);
...
void DICOMAppHelper::GetImagePositionPatientFilenamePairs(std::vector<std::pair<float, std::string>>& v, bool ascending)
{
	// Default to using the first series
	if (!this->Implementation->SeriesUIDMap.empty())
	{
...
void DICOMAppHelper::GetSeriesUIDs(std::vector<std::string>& v)
{
	...
	for (miter = this->Implementation->SeriesUIDMap.begin(); miter != this->Implementation->SeriesUIDMap.end(); ++miter)
	{
...
void DICOMAppHelper::PatientNameCallback(DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte)
{
	delete this->PatientName;
...
void DICOMAppHelper::StudyUIDCallback(DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte)
{
	delete this->StudyUID;
...
void DICOMAppHelper::StudyIDCallback(DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte)
{
	delete this->StudyID;
