...
void DICOMParser::ReadNextRecord(doublebyte& group, doublebyte& element, DICOMParser::VRTypes& mytype)
{
	...
	if (iter != Implementation->Map.end())
	{
		...
		//
		unsigned char* tempdata = reinterpret_cast<unsigned char*>(DataFile->ReadAsciiCharArray(length));
		...
		bool doSwap = (this->ToggleByteSwapImageData ^ this->DataFile->GetPlatformIsBigEndian()) && callbackType == VR_WordString;
		...
		for (std::vector<DICOMCallback*>::iterator cbiter = cbVector->begin(); cbiter != cbVector->end(); ++cbiter)
		{
...
void DICOMParser::InitTypeMap()
{
	DICOMRecord dicom_tags[] = {
		...
		{0x0018, 0x0050, DICOMParser::VR_Float},	 // slice thickness
		{0x0018, 0x0060, DICOMParser::VR_Float},	 // kV
		{0x0018, 0x0088, DICOMParser::VR_Float},	 // slice spacing
		{0x0018, 0x1100, DICOMParser::VR_SH},	 // Recon diameter
		{0x0018, 0x1151, DICOMParser::VR_Float},	 // mA
		...
		{0x0028, 0x0010, DICOMParser::VR_UnsignedShort},	 // Num rows
		{0x0028, 0x0011, DICOMParser::VR_UnsignedShort},	 // Num cols
		{0x0028, 0x0030, DICOMParser::VR_Float},	 // pixel spacing
		{0x0028, 0x0100, DICOMParser::VR_UnsignedShort},	 // Bits allocated
		{0x0028, 0x0120, DICOMParser::VR_UnsignedLong},	 // pixel padding
		{0x0028, 0x1052, DICOMParser::VR_Float},	 // pixel offset
		{0x7FE0, 0x0010, DICOMParser::VR_WordString}	 // pixel data
	};
	...
	for (int i = 0; i < num_tags; i++)
	{
		...
		Implementation->TypeMap.insert(std::pair<const DICOMMapKey, DICOMTypeValue>(DICOMMapKey(group, element), datatype));
	}
}
...
{
	return ((magic_number[0] == DICOM_MAGIC[0]) && (magic_number[1] == DICOM_MAGIC[1]) && (magic_number[2] == DICOM_MAGIC[2]) &&
			(magic_number[3] == DICOM_MAGIC[3]));
}

void DICOMParser::DumpTag(
	std::ostream& out, doublebyte group, doublebyte element, VRTypes vrtype, unsigned char* tempdata, quadbyte length)
{
	...
	doublebyte group, doublebyte element, VRTypes datatype, std::vector<DICOMCallback*>* cbVector)
{
	...
	{
		for (std::vector<DICOMCallback*>::iterator iter = cbVector->begin(); iter != cbVector->end(); ++iter)
		{
...
bool DICOMParser::ParseImplicitRecord(doublebyte group, doublebyte element, quadbyte& length, VRTypes& represent)
{
...
void DICOMParser::TransferSyntaxCallback(DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte)
...
void DICOMParser::GetGroupsElementsDatatypes(
	std::vector<doublebyte>& groups, std::vector<doublebyte>& elements, std::vector<DICOMParser::VRTypes>& datatypes)
{
	groups.clear();
	elements.clear();
	datatypes.clear();
	...
	for (giter = this->Implementation->Groups.begin(), eiter = this->Implementation->Elements.begin(),
		diter = this->Implementation->Datatypes.begin();
		 (giter != this->Implementation->Groups.end()) && (eiter != this->Implementation->Elements.end()) &&
...
void DICOMParser::ClearAllDICOMTagCallbacks()
{
	...
	for (mapIter = this->Implementation->Map.begin(); mapIter != this->Implementation->Map.end(); ++mapIter)
	{
