...
public:
        ...
        , TextureReference(&InOwner->TextureReference)
    {
        ...
        // This one line is the main point of this whole file - add TexCreate_UAV to the resource creation flags so we can target the
    ...
    virtual void InitRHI(FRHICommandListBase& RHICmdList) override
    {
        ...
        {
            // Make UAVs and SRVs to access all the mip levels.
            // TODO might only need SRVs if we only ever read/write from/to a single level - never readwrite?
            ...
            ShaderResourceViewRHIs.Add(
                FRHICommandListExecutor::GetImmediateCommandList().CreateShaderResourceView(TextureRHI,
                    FRHIViewDesc::CreateTextureSRV().SetDimensionFromTexture(TextureRHI).SetMipRange(i, 1)));
        }
...
public:
    ...
    // SRVs for each mip level - ordered from the largest (mip 0 = full quality) to the lowest (mip NumMips - 1)
    TArray<FShaderResourceViewRHIRef> ShaderResourceViewRHIs;
};