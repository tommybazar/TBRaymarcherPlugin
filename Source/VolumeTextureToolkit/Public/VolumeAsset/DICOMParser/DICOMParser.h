...
#include "VolumeAsset/DICOMParser/DICOMParserMap.h"
...
public:
	...
	enum VRTypes
	{
		...
		VR_WordString = 0x574f,	   // Other word string (sensitive to byte order, transfer syntax)
		...
		VR_Float = 0x4c46,	   // Floating point single precision
		VR_SignedLong = 0x4c53,	   // Signed long, (signed 32bits, 2's complement form.)
		VR_AT = 0x5441,	   // Attribute tag (ordered pair 16 bit unsigned)
		VR_UnsignedLong = 0x4c55,	   // Unsigned long (unsigned 32 bits)
		VR_UnsignedShort = 0x5355,	   // Unsigned short (unsigned 16 bits)
		VR_SignedShort = 0x5353,	   // Signed short, (signed 16bits, 2's complement)
		VR_Double = 0x4446	   // Floating point double precision
	};
	...
	void TransferSyntaxCallback(DICOMParser* parser, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte);

	void GetGroupsElementsDatatypes(
		std::vector<doublebyte>& groups, std::vector<doublebyte>& elements, std::vector<VRTypes>& datatypes);

protected:
	bool ParseExplicitRecord(doublebyte group, doublebyte element, quadbyte& length, VRTypes& represent);

	bool ParseImplicitRecord(doublebyte group, doublebyte element, quadbyte& length, VRTypes& represent);
	//
	// Print a tag.
	//
